...
- For AI meeting-note cleanup, Claude API, Claude Code CLI, Codex CLI, and Gemini CLI are supported runtime paths.
...
3. Complete the setup wizard:
   ...
   - configure Claude Code, Codex, or Gemini CLI for AI meeting-note cleanup,
...
3. AI cleanup creates a validated meeting-note draft and Markdown preview. This step supports Claude API plus Claude Code CLI, Codex CLI, and Gemini CLI terminal providers.
...
- STT용 설정 중 하나:
  ...
  - OpenAI STT와 API key.
- AI 회의록 정리는 Claude API, Claude Code CLI, Codex CLI, Gemini CLI provider를 지원합니다.
...
3. 설정 마법사를 완료합니다:
   ...
   - AI 회의록 정리용 Claude Code, Codex, Gemini CLI 설정,
...
3. AI cleanup이 검증된 회의록 초안과 Markdown preview를 생성합니다. 이 단계는 Claude API와 Claude Code CLI, Codex CLI, Gemini CLI 터미널 provider를 지원합니다.
