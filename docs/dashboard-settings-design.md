...
Phase 6C 구현은 Washed Blocks 방향을 유지하되 `dark`와 `light` palette를 분리한다.
`system`은 OS의 light/dark preference를 따라 해당 palette를 적용한다.
...
상태와 필수 필드 아래에 custom property rule 영역을 두고, 현재 구현에서는
meeting/member/task role별 rule을 분리 저장한다. 다만 Notion 필드 생성 적용과 고급
source 편집은 회의록 DB 중심으로 먼저 제공하고, 작업자/액션 아이템 DB의 source 확장은
후속 단계로 연결한다.
...
- MVP에서는 회의록 DB의 `AI` source와 Notion 필드 생성 적용을 우선 제공한다.
- 후속으로 작업자/액션 아이템 DB의 고급 source 편집, `고정값`, `액션 아이템 전용 source`,
  `참가자 기반 relation`을 확장한다.
...
Dirong은 사용자의 음성 데이터를 처리하는 운영형 서비스를 계획하지 않는다. 음성
원본과 STT 텍스트는 사용자가 실행 중인 PC에 저장된다. 대시보드는 저장 위치,
보관 정책, 삭제 이력을 명확히 보여줘야 한다.
...
- `system`, `light`, `dark` theme 값은 유지한다. `dark`는 Washed Blocks 기반 다크
  palette를 쓰고, `light`는 같은 구조의 라이트 palette를 쓴다. `system`은 OS
  preference에 맞춰 전환한다.
...
- theme 값은 `system`, `light`, `dark`를 지원하고 local settings에 저장한다. 현재 시각
  구현에서는 `dark`와 `light` palette가 분리되어 있고, `system`은 OS preference를 따른다.
