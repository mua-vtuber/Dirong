...
- 기존 DB 가져오기와 자동 마이그레이션은 고급 기능으로 남긴다. 여기서 기존 DB는
  디롱이가 parent page 아래 새로 만든 managed DB가 아니라, 사용자가 이미 Notion에
  만들어 둔 DB를 의미한다.
- 작업자/액션 아이템 DB의 고급 source 편집과 Notion 필드 생성 적용은 후속 단계로 둔다.
- 액션 아이템 상태 자동 완료, 멘션/알림, Notion OAuth는 MVP 범위 밖이다.
- 사용자 음성 데이터를 처리하는 운영형 서비스는 제품 방향에서 제외한다.
