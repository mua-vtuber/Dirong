    ...
    function renderNotionConnectionSettings(setup) {
      ...
      const parentPageUrl = setup?.editableSettings?.notion?.parentPageUrl ?? '';
      return '<div class="metric"><div class="label">' + i18n('dashboard.settings.tabs.notion') + '</div>' +
        renderHumanDisplay(feature) +
        renderRuntimeEffect(feature?.runtimeEffect) +
        '<div class="muted">' + i18n('dashboard.settings.secretsHidden') + '</div></div>' +
        '<div class="metric"><div class="label">' + i18n('dashboard.settings.editor.notion.title') + '</div>' +
