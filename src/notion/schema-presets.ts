...
export function validateNotionSchemaPreset(
  ...
  validateTaskStatusOptions(propertyByKey, errors);
...
function validateTaskStatusOptions(
  ...
  if (!status || !sameStringList(status.options ?? [], NOTION_TASK_STATUS_OPTIONS)) {
    errors.push({
      code: "invalid_task_status_options",
      message: `task.status options must be ${NOTION_TASK_STATUS_OPTIONS.join(", ")}.`,
      databaseRole: "task",
